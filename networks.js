...
function txt(){
  var t = "memetic networks reflect power structures".split("").join(String.fromCharCode(0x2004));
  $.font = "2.5em sans-serif";
  $.fillStyle = 'hsla(360, 100%, 50%, 1)';
  $.fillText(t, (c.width - $.measureText(t).width) * 0.5, c.height * 0.5);
}

function draw() {
 $.fillStyle = 'hsla(247, 88%, 49%, 1)';
...
function conn(p1, p2) {
  $.strokeStyle = 'hsla(360, 100%, 50%, 1)';
  var dist = _dist(p1, p2);
  $.globalAlpha = 10 - dist / 100;
  $.beginPath();
  $.moveTo(p1.x, p1.y);
  $.lineTo(p2.x, p2.y);
  $.stroke();
